...
- **Public vs. Private Cluster**:
  - A Public Cluster allows Internet traffic to the cluster API and `*.apps` endpoints. Suitable for applications needing public internet access.
  - A Private Cluster is the option to make the cluster API and `*.apps` endpoints private. Use Azure Frontdoor for Internet access to applications on a private cluster. This enhances security by keeping the cluster and Azure resources private, managing traffic at the edge, and providing benefits like WAF, DDoS protection, SSL management, and offloading. For detailed implementation guidance, refer to [Azure Frontdoor documentation](/experts/aro/frontdoor/).
- **Egress Lockdown**:
