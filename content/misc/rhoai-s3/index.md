...
tags: ["ROSA", "RHOAI", "Jupyter", "LLM", "TensorFlow", "S3"]
authors:
  - Diana Sari
---
...
This is a simple example on how to run and deploy LLMs on a [Red Hat OpenShift (ROSA)](https://www.redhat.com/en/technologies/cloud-computing/openshift/aws) cluster, which is our managed service OpenShift platform on AWS, using [Red Hat OpenShift AI (RHOAI)](https://www.redhat.com/en/technologies/cloud-computing/openshift/openshift-ai), which is formerly called Red Hat OpenShift Data Science (RHODS) and is our OpenShift platform for managing the entire lifecycle of AI/ML projects. And we will utilize [Amazon S3](https://aws.amazon.com/s3/) bucket to store the model output.

In this guide, we will first install RHOAI operator, and run XXXX
...
Next, go to your cluster console and click the 9-boxes icon on384524
 upper right side (next to the bell/notification icon), and select **Red Hat OpenShift AI** to launch it on the next tab. 
...
```
oc exec -it <jupyter-nb-pod-name> -- /bin/bash
```
...
Once the credentials matter is sorted, let's create an S3 bucket in your AWS account. Again, there are many ways to go about this. The easiest would be to go to your AWS console and create the bucket in your region from there and leave all the settings to default. Alternatively, you can run this command to create a bucket:
```
...
## 7. Performing hyperparameter tuning (bonus section)
