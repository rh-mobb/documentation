...
| Error | Cause | Fix |
|-------|-------|-----|
| `Domain 'X' is incompatible with architecture parent domain 'p3.openshiftapps.com'` | `base_dns_domain` is not under `p3.openshiftapps.com` | Run `rosa create dns-domain --hosted-cp` to get an HCP-compatible domain |
| `Attribute 'dns.base_domain' is mandatory on shared VPC clusters` | `base_dns_domain` is missing | Pass `--base-domain` to `rosa create cluster` |
| `failed to find dns domain 'X' for organization` | Domain not registered with OCM | Run `rosa create dns-domain --hosted-cp` |
| `Failed to assume role ... AccessDenied` (Installer Role) | Trust policy on shared VPC role is missing the Installer Role ARN | Update the trust policy (Step 9) |
| `Failed to assume role ... AccessDenied` (operator) | Operator role missing `sts:AssumeRole` inline policy | Recreate operator roles with `--route53-role-arn` and `--vpc-endpoint-role-arn` flags |
| `not authorized to access hosted zone` | Missing Route 53 permissions | Verify `ROSASharedVPCRoute53Policy` is attached to the route53 role |
| `Hosted zone name should match the pattern` | Zone name doesn't follow ROSA naming | Use `<cluster>.hypershift.local` and `rosa.<cluster>.<base_domain>` |
| `HostedZoneNotEmpty` on destroy | Leftover DNS records from a failed cluster | Manually delete non-NS/SOA records, then retry |
| Cluster stuck in `installing` | IAM propagation delay or role misconfiguration | Check `rosa logs install -c <cluster>` and verify all role ARNs |

---

## Important Notes

1. **IAM propagation matters.** AWS IAM changes are eventually consistent (5–15 seconds). The `sleep 15` after updating trust policies in Step 9 prevents `AccessDenied` errors during cluster creation.

2. **Private cluster access.** All `oc` commands require the bastion tunnel (sshuttle or SOCKS proxy) since the API server is private.

3. **Hosted zone naming is strict.** ROSA HCP validates zone names during cluster creation. The HCP internal zone must be `<cluster>.hypershift.local` and the ingress zone must be `rosa.<cluster>.<base_domain>`. Any deviation causes creation to fail.

4. **DNS domain must be `p3`.** ROSA HCP requires `p3.openshiftapps.com` domains. Classic ROSA domains (`p1`) or custom domains are not compatible.

5. **Single VPC per cluster.** Multiple ROSA clusters in a single VPC are not supported. Each cluster needs its own VPC.

6. **Break-glass recovery.** If you lose access to the cluster (e.g., IdP misconfiguration), use `rosa create admin -c <cluster>` to create a temporary admin via the ROSA CLI (requires only the OCM token, not cluster credentials).

---

## Terraform Automation (Optional)

All the steps above can be automated using Terraform with the [RHCS provider](https://registry.terraform.io/providers/terraform-redhat/rhcs/latest/docs). A complete Terraform automation is available in the `terraform/` directory of this repository.

### Module Structure

```
terraform/
├── providers.tf            # AWS (default + shared_vpc_account alias), RHCS
├── variables.tf            # All input variables
├── main.tf                 # Module orchestration + operator IAM policies
├── outputs.tf              # Cluster URLs, KMS ARN, cluster ID
├── modules/
│   ├── shared-vpc/         # VPC, subnets, NAT, hosted zones, cross-account IAM roles
...
│   ├── rosa-entra-idp/     # (Optional) Microsoft Entra ID OIDC IdP
│   └── rosa-post-install/  # KMS-encrypted StorageClasses
└── environments/
    └── us-east-1/
        └── demo.tfvars     # Example variable values
```

### Quick Start with Terraform

**Step 0 (prerequisite):** Create account roles, OIDC config, operator roles, and DNS domain using the `rosa` CLI as described in Steps 5–8 above. Terraform references these via data sources.
...
# Stage 1: VPC + IAM (targets shared_vpc module + operator inline policies)
terraform apply -var-file=environments/us-east-1/demo.tfvars \
...
Cluster creation takes 20–30 minutes. After it completes, set up bastion access (Step 12), log in (Step 13), then apply post-install resources:
...
```bash
terraform destroy -var-file=environments/us-east-1/demo.tfvars
rosa delete operator-roles --prefix $ROLES_PREFIX --mode auto -y
rosa delete oidc-config --oidc-config-id $OIDC_CONFIG_ID --mode auto -y
rosa delete account-roles --prefix $ROLES_PREFIX --mode auto -y
```
