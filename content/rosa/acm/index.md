...
6. An OCM Service Account ClientID and Client Secret - if you don't have one, you can create a service account by visiting [here](https://console.redhat.com/iam/service-accounts)<br><br>
...
7. Set environment variables
   ...
   export ROSA_API_SERVER=$(oc config view --minify -o jsonpath='{.clusters[*].cluster.server}')
   export OCM_CLIENT_ID= # SERVICE ACCOUNT CLIENT ID
   export OCM_CLIENT_SECRET= # SERVICE ACCOUNT CLIENT SECRET
   ```

8. Create a scratch directory for temporary files

   ```bash
   export SCRATCH_DIR="${HOME}/acm-rosa-scratch"
   mkdir -p "${SCRATCH_DIR}"
   cd "${SCRATCH_DIR}"
   ```
...
2. Create a MultiClusterHub Instance
   ...
   Before continuing, wait for the multicluster engine to complete. This can take 10-15 minutes.

   ```bash
   oc wait --for=jsonpath='{.status.phase}'='Running' MultiClusterHub multiclusterhub -n open-cluster-management --timeout=20m
   ```

   If the command times out, check the status with:
...
1. Create a trust policy
   ...
   ```bash
   cat <<EOF > "${SCRATCH_DIR}/trust.json"
   {
     "Version": "2012-10-17",
     "Statement": [
       {
...
2. Create a role with a trust policy for RHACM to use
   ...
   ```bash
   CAPA_ROLE=$(aws iam create-role --role-name "capa-manager-role-${ACM_CLUSTER_NAME}" --assume-role-policy-document file://"${SCRATCH_DIR}/trust.json" --description "AWS Role for CAPA to assume" --query "Role.Arn" --output text)
   echo $CAPA_ROLE
   ```
...
4. Deploy AWS VPC and Subnets for the cluster
   ...
   >Note: This example deploys ROSA to a single availability zone but can be easily adopted for multi-zone configuration
   ...
   export PRIVATE_SUBNET=$(terraform output | grep -A 1 "cluster-private-subnets" | tail -n 1 | cut -d'"' -f2)
   export PUBLIC_SUBNET=$(terraform output | grep -A 1 "cluster-public-subnets" | tail -n 1 | cut -d'"' -f2)
